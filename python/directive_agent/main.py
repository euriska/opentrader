...
class DirectiveAgent(BaseAgent):
    ...
    async def _evaluate_one(self, directive: dict) -> tuple[bool, str]:
        ...
        # Gather live market context for any tickers mentioned
        import re
        tickers = list(set(re.findall(r'\b([A-Z]{2,5})\b', text)))[:5]
        market_context = ""
        for ticker in tickers:
            try:
                tv = await get_tv_indicators(ticker)
                if tv:
                    # Also get current price via broker gateway
...
{{
  ...
  "action": {{
    ...
    "quantity": <integer shares or null for dollar-based>,
...
"""
        ...
        if not ticker:
            return False, "LLM returned should_execute=true but no ticker specified"

        # Resolve quantity
        if not quantity and dollars:
            price = await self._get_price(ticker)
            if price and price > 0:
                quantity = max(1, int(dollars / price))
            else:
                quantity = 1

        quantity = max(1, int(quantity or 1))

        # Place order via broker gateway for each active account
        assignments = load_active_assignments("equity") or []
        if not assignments:
        ...
        for assignment in assignments:
            ...
            try:
                ...
                status = r.get("status", "")
                if status != "error":
                    executed_accounts.append(account_label)
            except Exception:
                pass

        if not executed_accounts:
            return False, f"Order placement failed for {ticker} on all accounts"

        reasoning = result.get("reasoning", "")
        summary = (
            f"Executed: {ticker} {direction.upper()} {quantity} shares "
